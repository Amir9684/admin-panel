...
const ExpandableTable = ({ data }) => {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      await apiCall.post("/News/CreateNewsReplyComment", body).then((res) => {
...
export const columns = [
  {
    name: "کاربر",
    minWidth: "250px",
    sortable: (row) => row.autor,
    cell: (row) => (
      <>
        {row.replyCount > 0 ? (
          <Link
            to={`/courses-comments/${row.newsId}/${row.id}`}
            className="d-flex align-items-center"
          >
            <div className="user-info text-truncate ms-1">
              <span className="d-block fw-bold text-truncate">{row.autor}</span>
            </div>
          </Link>
        ) : (
  ...
  },
  {
    ...
    sortable: true,
    minWidth: "150px",
    selector: (row) => getPersianNumbers(row.likeCount),
  },
  {
    name: "دیسلایک‌ها",
    sortable: true,
    minWidth: "150px",
    selector: (row) => getPersianNumbers(row.dissLikeCount),
  },
  // {
  //   name: "عملیات",
  //   allowOverflow: true,
